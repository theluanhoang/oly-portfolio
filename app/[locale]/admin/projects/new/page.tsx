...
export default function NewProjectPage() {
  ...
  useEffect(() => {
    // chỉ reset giá trị chọn, không đụng vào danh sách subCategories (đã flatten)
    setSubCategoryInput('');
  ...
  const handleAddSubCategory = () => {
    ...
    const existing = availableSubCategories.find(
      (s) => s.name.toLowerCase() === name.toLowerCase() && s.categoryId === catId
    );
    if (existing) {
  ...
  const onSubmit = async (data: ProjectSchema) => {
    ...
    try {
      ...
      // Ensure category/subcategory exist or create on the fly
      ...
      // Helper to create category if needed
      ...
      // Helper to create subcategory if needed
      if (!finalSubCategoryId && subCategoryInput.trim()) {
        // Need a category to attach
        if (!finalCategoryId) {
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6" key={currentLocale}>
                    ...
                    <div className="relative">
                      <input
                        ...
                        placeholder={selectedCategoryId ? t('fields.selectSubCategory') : t('fields.selectCategoryFirst')}
                        className="w-full h-[44px] border border-black px-4 pr-10 rounded-lg bg-white text-[#333] outline-none focus:outline-none focus:border-black transition-colors"
