...
export default function EditProductPage() {
  ...
  useEffect(() => {
    async function loadProduct() {
      try {
        ...
        const translationsObj: Record<string, { title: string; descriptions: string[]; content: string }> = {};
        if (product.translations && Array.isArray(product.translations)) {
          product.translations.forEach((trans: { locale: string; title: string; descriptions: string[]; content: string }) => {
        ...
        // Ensure vi and en exist
        if (!translationsObj.vi) {
          translationsObj.vi = { title: '', descriptions: [], content: '' };
        }
        if (!translationsObj.en) {
          translationsObj.en = { title: '', descriptions: [], content: '' };
        }
        
        // Get category and material names for display
        const categoryTranslation = product.category?.translations?.find((t: { locale: string }) => t.locale === currentLocale)
          || product.category?.translations?.find((t: { locale: string }) => t.locale === 'vi')
          || product.category?.translations?.find((t: { locale: string }) => t.locale === 'en')
          || product.category?.translations?.[0];
        
        const materialTranslation = product.material?.translations?.find((t: { locale: string }) => t.locale === currentLocale)
          || product.material?.translations?.find((t: { locale: string }) => t.locale === 'vi')
          || product.material?.translations?.find((t: { locale: string }) => t.locale === 'en')
        ...
        // Set input values
        if (product.categoryId && categoryTranslation) {
          setCategoryInput(categoryTranslation.name || '');
        }
        if (product.materialId && materialTranslation) {
          setMaterialInput(materialTranslation.name || '');
        }

        // Set previous titles for slug generation
        previousTitleRef.current = {
          vi: translationsObj.vi?.title || '',
          en: translationsObj.en?.title || '',
        };
      } catch (error) {
  ...
  // Handlers for category and material
