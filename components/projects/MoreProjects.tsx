...
export default function MoreProjects({
  ...
  const topOffset = headerHeight + 12; // navbar height + 12px breathing room
  const { windowWidth } = useResponsive();
  const isLargeScreen = windowWidth >= 1024; // matches Tailwind lg: breakpoint
  ...
  return (
    <StickyBox topOffset={topOffset}>
      {/* Title — stays fixed, not affected by scroll */}
      <h2 className="text-black text-[32px] font-bold leading-normal tracking-[4.48px] uppercase mb-[30px]">
        {t('moreProject')}
      </h2>
      {/* Project list — scrollable on desktop, normal on mobile */}
      <div
        className={isLargeScreen ? "more-projects-scroll overflow-y-auto" : ""}
        style={
          isLargeScreen
            ? ({ maxHeight: `calc(100vh - ${topOffset + 90}px)`, scrollbarWidth: "none", msOverflowStyle: "none" } as React.CSSProperties)
            : undefined
        }
      >
